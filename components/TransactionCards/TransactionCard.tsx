...
const TransactionCard = ({ transactionDetails, categories }: Transactions) => {
  ...
  const handleCategorySelection = (
    ...
    if (isChecked) {
      ...
      setSelectedCategories((prevCategories) =>
        (prevCategories as string[]).filter((id) => id !== categoryId)
      );
    }
  };
  ...
  return (
    <div
      className={`w-[250px] h-fit bg-white border mx-8 my-8 rounded-lg ${JetBrains.className}`}
    >
      ...
      <div className="text-center mt-4 text-[16px] px-4 gap-4 ">
        <span className="text-[12px]">KES</span>
        {transactionDetails.investmentAmount}
      </div>
      ...
      <div className="mt-5 font-bold  w-full h-[50%] rounded-lg bg-black border-2 text-white px-4 py-2">
        ...
        <div>
          Returns :{" "}
          {transactionDetails.return ? (
            <>{transactionDetails.return.toFixed(2)}</>
          ) : (
            <>N.A.</>
          )}
        </div>
        <div>
          Total :{" "}
          {transactionDetails.totalReturns ? (
            <>{transactionDetails.totalReturns.toFixed(2)}</>
          ) : (
            <>N.A.</>
          )}
        </div>
        ...
        <div className="mt-8 flex justify-between w-full items-center mb-2">
          <div
            className="rounded-lg px-1 py-1 border-2 hover:bg-white hover:text-black cursor-pointer transition duration-500"
            onClick={togglePopup}
          >
            <AiOutlinePlus />
          </div>
          ...
          <div
            className="rounded-lg px-3 py-1 border-2 text-[14px] select-none hover:bg-white hover:text-black transition duration-500 cursor-pointer"
            onClick={() => {
              router.push("/fixed-deposit");
            }}
          >
            Calculator
          </div>
                  ...
                  <li key={category.id}>
                    <label className="flex items-center">
                      <input
                        ...
                        value={category.id}
                        onChange={(e) =>
                          handleCategorySelection(e, category.id)
                        }
