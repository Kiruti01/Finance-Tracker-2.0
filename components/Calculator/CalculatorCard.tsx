...
const CalculatorCard = ({ calculatorDetails }: CalculatorProps) => {
  ...
  const handleCopyLink = () => {
    const url = `finance-tracker-2-0.vercel.app/calculator/${calculatorDetails.calculatorName}`;
    navigator.clipboard
      .writeText(url)
      .then(() => {
  ...
  return (
    ...
    >
      <div
        className={`w-[220px] h-[300px] bg-white border mx-8 my-8 rounded-lg ${JetBrains.className}`}
      >
        ...
        <div className="mt-5 font-bold h-[170px]  w-full rounded-lg bg-black border-2 text-white px-4 py-2 flex flex-col justify-between mb-2">
          ...
          <div className="mt-8 flex justify-between  w-full mb-1">
            <div
              className={`rounded-lg px-3 py-1 border-2 text-[14px] select-none hover:bg-white hover:text-black transition duration-500 cursor-pointer items-center flex justify-center`}
            ...
            <div
              ...
              onClick={() => {
                router.push(`calculator/${calculatorDetails.calculatorLink}`);
              }}
            >
              Calculator
            </div>
